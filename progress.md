...
## 2026-09-12 视频成果提交与媒体设计审阅

- 已按指定信息 `feat: implement video declaration minimal chain (V22/V6, DECLARED)` 提交全部 26 个成果文件，代码提交 `da6eaeb7b82b1cb0d03f75c36b0e6423e9e7f028`；已推送 `origin/codex/p6-2-ops-safety-gates`，ls-remote 确认相同 SHA，提交后工作区干净。上文“未提交／未推送”为历史状态，本节取代。
- 本轮重新核对既有报告：相关回归 34/34、隔离 PG 2/2；未重跑测试、启动数据库或媒体服务，未构建／推送镜像。此后仅新增本轮评估／设计文档并更新本记录；文档单独提交，不包含媒体实现。
- Hibernate：已复现旧 V13 `jt_gateway_audit_events.payload_digest` CHAR/VARCHAR JDBC 映射差异；另盘点 5 个 CHAR 摘要／哈希候选及空间列声明差异。可保持既有 ddl-auto=none 继续媒体准备，建议媒体 PG 整体验收前用 2–4 人日修复实体映射并定向回归；全库 validate 尚未通过。方案 `docs/pilot/p6-2-c1-hibernate-validate-assessment.md`，本轮未修代码／配置。
- 媒体稿 `docs/pilot/p6-2-c1-media-closed-loop-minimal-design.md`：推荐 ZLMediaKit 开源核心加独立 JT1078／附件接收适配。ZL 原生 JT1078 属闭源专业版；EasyCVR 按需授权产品评估；SRS 可作适配后分发备选。本轮仅查公开官方资料，未连接厂商演示系统。
- 报警取证建议终端预录上传。现有流程因不保留原始 16 字节报警标识而固定拒绝，须单独审阅受控保留和存储扩展；不能只替换媒体适配器。本设计未批准实施，未新增或修改迁移、队列白名单、manifest、云配置。
