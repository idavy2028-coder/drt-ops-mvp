...
后续 Task12 隔离验收使用 [Task12 本地安全门禁手册](p6-2-task12-local-safety-runbook.md) 中的版本化入口：
验收按四个物理终端逐项核对，同车双设备不重复算车辆；一次性资源清理默认仅预检，实际删除必须核对创建时 receipt、资源标签和完整身份。
不得复用旧 seed helper 的数量或命名测试来替代这两个门禁，也不得根据现有资源名称补造 receipt 后删除。
本地门禁不授予云端部署或真实流量权限；其他章节中的部署步骤继续遵循各自授权边界。
