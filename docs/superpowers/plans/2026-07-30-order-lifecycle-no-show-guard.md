...
```powershell
.\.tools\apache-maven-3.9.11\bin\mvn.cmd -q -pl apps/api -Dtest=NoShowEligibilityPolicyTest test
```
...
- [ ] **Step 6: 运行接口和相关任务测试并确认 GREEN**

```powershell
.\.tools\apache-maven-3.9.11\bin\mvn.cmd -q -pl apps/api -Dtest=OrderExceptionApiTest,TaskExecutionApiTest test
```
...
```powershell
.\.tools\apache-maven-3.9.11\bin\mvn.cmd -q -pl apps/api -Dtest=RideOrderApiTest test
```
