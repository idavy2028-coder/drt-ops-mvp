...
<template>
  <section class="terminal-page" aria-label="终端管理">
    <aside class="compatibility-banner" aria-label="物理设备兼容操作">
      <div><strong>物理设备兼容操作</strong><span>这里保留既有终端预置、绑定、换机与安全操作。</span></div>
      <a href="/terminals">返回车载系统总览</a>
    </aside>
    ...
    <div class="layout">
      ...
      <article v-else-if="selected" class="detail">
        ...
        <div class="cards">
          ...
          <section><h4>链路时间</h4><p>当前会话：{{ selected.onlineStatus === "ONLINE" ? "当前：在线且已鉴权" : "当前：离线" }}</p><p>最近注册：{{ time(selected.lastRegisteredAt) }}</p><p>最近成功鉴权（历史）：{{ time(selected.lastAuthenticatedAt) }}</p><p>会话最近有效消息：{{ time(selected.lastValidMessageAt) }}</p><p>心跳：{{ time(selected.lastHeartbeatAt) }}</p><p>位置：{{ time(selected.lastLocationAt) }}</p><p>离线判定：{{ time(selected.offlineAt) }}</p></section>
          <section><h4>当前车载系统归属</h4><p v-if="selected.currentOnboardMembership">{{ selected.currentOnboardMembership.plateNumber }} · {{ selected.currentOnboardMembership.status }}</p><p v-else>尚未归属</p></section>
          <section><h4>历史 legacy 绑定</h4><p v-if="selected.legacyBindingHistory.length === 0">尚无历史</p><ul v-else><li v-for="binding in selected.legacyBindingHistory" :key="`${binding.plateNumber}-${binding.validFrom}`">{{ binding.plateNumber }} · {{ binding.status }} · {{ time(binding.validFrom) }}</li></ul></section>
        </div>
