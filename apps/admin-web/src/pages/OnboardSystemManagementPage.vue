...
async function loadSystems(
  ...
  try {
    ...
    if (page.items.length === 0 && page.totalPages > 0 && page.page >= page.totalPages) {
      if (allowEmptyPageFallback) {
        // 数据收缩后的越界页最多回退一次，避免异常服务端元数据造成递归请求。
        await loadSystems(page.totalPages - 1, false);
        return;
      }
    ...
    currentPage.value = page.totalPages === 0 ? 0 : page.page;
    totalElements.value = page.totalElements;
    totalPages.value = page.totalPages;
    ...
    const nextSelection = page.items.find((system) => system.vehicleId === previousSelection)
      ?? page.items[0];
    if (nextSelection) {
      void selectSystem(nextSelection.vehicleId);
    } else {
      selectedVehicleId.value = "";
      detail.value = null;
      draft.value = null;
    ...
    if (request === listRequestToken) loading.value = false;
  }
}
...
async function changePage(nextPage: number): Promise<void> {
  if (applying.value
      ...
      || nextPage === currentPage.value) return;
  if (draft.value) {
    draft.value = null;
...
async function applyConfiguration(): Promise<void> {
  ...
  // apply 开始后作废此前已发出的列表请求，防止其晚回覆盖权威应用状态。
  listRequestToken += 1;
  loading.value = false;
  applying.value = true;
  pageError.value = "";
  ...
  <section class="onboard-page" aria-label="车载系统管理">
    ...
    <div class="system-layout">
      <aside class="system-list" aria-label="车载系统列表">
        <div class="list-heading"><strong>车辆</strong><span>共 {{ totalElements }} 套系统</span></div>
        <button
        ...
        <nav class="pagination" aria-label="车载系统分页">
          ...
          <span aria-live="polite">第 {{ totalPages === 0 ? 0 : currentPage + 1 }} / {{ totalPages }} 页</span>
          <button type="button" aria-label="下一页"
            :disabled="loading || applying || currentPage + 1 >= totalPages"
...
<style scoped>
.pagination { display:grid; grid-template-columns:1fr auto 1fr; align-items:center; gap:6px; padding-top:5px; }.system-list .pagination button { display:block; padding:7px 8px; text-align:center; }.pagination span { color:var(--ink-muted); font-size:12px; white-space:nowrap; }
