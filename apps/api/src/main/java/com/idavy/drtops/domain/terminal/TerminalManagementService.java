...
public class TerminalManagementService {
    ...
    public JtTerminal completeRegistration(
        ...
        JtTerminal terminal = terminalRepository.findById(terminalId)
                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "terminal not found"));
        terminal.completeRegistration(tokenVersion, tokenSha256);
    ...
    public RegistrationDecision verifyRegistration(
        ...
        if (terminal == null || terminal.getStatus() == JtTerminal.Status.RETIRED
    ...
    public ActionResult suspend(String terminalCode, long expectedVersion, String reason, UUID actorId) {
        ...
        return disconnectAndAudit(terminal, "TERMINAL_SUSPENDED", actorId, reason);
    }
    ...
    public ActionResult retire(String terminalCode, long expectedVersion, String reason, UUID actorId) {
        ...
        return disconnectAndAudit(terminal, "TERMINAL_RETIRED", actorId, reason);
    }
    ...
    public ReplacementResult replace(
        ...
        requireReason(reason);
        if (replacement.getLastRegisteredAt() == null || replacement.getStatus() != JtTerminal.Status.PENDING) {
        ...
        asConflict(oldTerminal::retire);
        replacement.touch();
        bindingRepository.save(JtTerminalVehicleBinding.bind(
                replacement, oldBinding.getVehicleId(), reason, actorId));
        terminalRepository.saveAndFlush(oldTerminal);
        terminalRepository.saveAndFlush(replacement);
        ActionResult disconnect = disconnectAndAudit(oldTerminal, "TERMINAL_REPLACED", actorId, reason);
        audit(oldTerminal, "JT_TERMINAL_REPLACED", actorId, reason);
        return new ReplacementResult(replacement, disconnect.disconnectStatus());
    }

    @Transactional
    public JtTerminal rotateAuthentication(
            String terminalCode,
            long expectedVersion,
            int tokenVersion,
            String tokenSha256,
        ...
        JtTerminal terminal = requireVersion(terminalCode, expectedVersion);
        requireReason(reason);
        terminal.rotateAuthentication(tokenVersion, tokenSha256);
        terminalRepository.saveAndFlush(terminal);
    ...
    public ActionResult disconnect(
        ...
        terminal.touch();
        terminalRepository.saveAndFlush(terminal);
        return disconnectAndAudit(terminal, "OPERATOR_REQUESTED", actorId, reason);
    }

    private ActionResult disconnectAndAudit(
            JtTerminal terminal, String reasonCode, UUID actorId, String reason) {
        boolean confirmed = controlClient.disconnect(terminal.getId(), reasonCode);
        audit(terminal, "JT_TERMINAL_DISCONNECT_REQUESTED", actorId, reason);
        return new ActionResult(terminal, confirmed ? "DISCONNECT_CONFIRMED" : "DISCONNECT_PENDING_CONFIRMATION");
    }
    ...
    private void audit(JtTerminal terminal, String action, UUID actorId, String reason) {
        auditLogRepository.save(AuditLog.record(
                "JT_TERMINAL", terminal.getId(), action, "USER", actorId.toString(), reason, "{}"));
    }
